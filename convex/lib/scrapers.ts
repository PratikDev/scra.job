...
import { onlyRemoteJobs } from "./remoteFilter";
...
function parseRemoteOkDate(job: RemoteOkJob) {
	if (job.epoch) return Number(job.epoch) * 1000;
...
function parseTitleCompany(rawTitle: string) {
	...
	if (atMatch) return { title: cleanText(atMatch[1]), company: cleanText(atMatch[2]) };

	const dashMatch = title.match(/^(.*?)\s+-\s+(.+)$/);
	if (dashMatch) return { title: cleanText(dashMatch[2]), company: cleanText(dashMatch[1]) };

	const colonMatch = title.match(/^([^:]{2,80}):\s+(.+)$/);
	if (colonMatch) return { title: cleanText(colonMatch[2]), company: cleanText(colonMatch[1]) };
...
export async function scrapeWeWorkRemotely(): Promise<ScrapedJobPayload[]> {
	const data = await fetchText("https://weworkremotely.com/remote-jobs.rss");
	const $ = cheerio.load(data, { xmlMode: true });
	const jobs = $("item")
		.toArray()
		.slice(0, 40)
		.map((item): ScrapedJobPayload | null => {
			const node = $(item);
		...
		.filter((job): job is ScrapedJobPayload => Boolean(job?.title && job.url));

	return onlyRemoteJobs(jobs);
}

export async function scrapeRemoteOk(): Promise<ScrapedJobPayload[]> {
	const data = await fetchJson<RemoteOkJob[]>("https://remoteok.com/api");
	const rows = Array.isArray(data) ? data.slice(1, 51) : [];
	const jobs = rows
		.map((job): ScrapedJobPayload => ({
			source: "RemoteOK",
...
export async function scrapeYCombinatorSoftwareEngineerJobs(): Promise<ScrapedJobPayload[]> {
	const data = await fetchText("https://www.ycombinator.com/jobs/role/software-engineer", {
...
export async function scrapeHackerNewsJobsPage(): Promise<ScrapedJobPayload[]> {
	const data = await fetchText("https://news.ycombinator.com/jobs");
	const $ = cheerio.load(data);
	const jobs = $(".athing")
		.toArray()
		.map((row): ScrapedJobPayload | null => {
			const node = $(row);
...
export async function scrapeHackerNewsJobs(): Promise<ScrapedJobPayload[]> {
	const ids = await fetchJson<number[]>("https://hacker-news.firebaseio.com/v0/jobstories.json");
	const topIds = Array.isArray(ids) ? ids.slice(0, 30) : [];
	const responses = await Promise.allSettled(
		topIds.map((id) => fetchJson<HackerNewsJob>(`https://hacker-news.firebaseio.com/v0/item/${id}.json`))
	);
	const jobs = responses
		.filter((result): result is PromiseFulfilledResult<HackerNewsJob> => result.status === "fulfilled" && Boolean(result.value))
		.map((result): ScrapedJobPayload => {
			...
			return {
				...
				datePosted: item.time ? item.time * 1000 : null,
				salaryRange: null,
			};
		})
...
export async function scrapeAllSources(): Promise<ScrapeResult> {
	const sources: [string, () => Promise<ScrapedJobPayload[]>][] = [
	...
	results.forEach((result, index) => {
		...
		if (result.status === "fulfilled") {
			...
			errors.push({ source, message: result.reason?.message || "Scrape failed" });
		}
	});
...
export async function scrapeJobFromUrl(url: string) {
